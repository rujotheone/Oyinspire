...
class Auth
{
	...
	*/
	...
	public function register($email, $username, $password, $repeatpassword, $params = Array(), $captcha = NULL, $sendmail = NULL)
	{
		$return['error'] = true;
		...
		//my functions
		if ($this->getUIDFromUsername($username)!=false)
		{
			$return['error']=1;
			$return['message']="This username has been taken";
		    return $return;
		}
		$addusername = $this->setUsername($username,$email);
		...
		{
			$return['error']=1;
			$return['message']="There was a problem adding username";
			return $return;
		}
...
//my functions
    ...
    {
    	$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET username = ? WHERE email =?");
